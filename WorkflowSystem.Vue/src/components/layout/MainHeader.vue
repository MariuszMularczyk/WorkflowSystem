<template>
  <header>
    <nav>
      <h2>
        <router-link to="/" draggable="false">WorkFlow</router-link>
      </h2>
      <ul class="nav-menu" v-bind:class="[isMobileMenuOpened ? 'active' : '']">
        <li v-if="isLoggedIn && getLoggedInUser.userType != 10">
          <router-link
            ...
            to="/mytasks"
            draggable="false"
            >Obieg Faktur</router-link
          >
        </li>
        <li v-if="isLoggedIn && getLoggedInUser.userType != 10">
          <router-link to="/dashboard" draggable="false">Dashboard</router-link>
        </li>
        <li v-if="isLoggedIn && getLoggedInUser.userType != 10">
...
export default {
  ...
  computed: {
    ...
    checkForPlayersPath() {
      return this.$route.fullPath.includes("/users") ? true : false;
    },
  },
  methods: {
    ...mapActions({
