...
export default {
  ...
  actions: {
    setMessages({ commit }, userId) {
      axios
        .get("https://localhost:44379/api/messages", {
    ...
    setSentMessages({ commit }, userId) {
      axios
        .get("https://localhost:44379/api/messages/getSentMessages", {
          params: { userId: userId },
          headers: authHeader(),
        })
        .then((response) => {
    ...
    sendMessage: async ({ state, dispatch }) => {
      ...
      await axios
        .post(
          "https://localhost:44379/api/messages/addMessage",
          state.message,
          {
            headers: authHeader(),
          }
    ...
    setMessageTypesToLookup: () => {
      return new Promise((resolve, reject) =>
        axios
          .get("https://localhost:44379/api/messages/getMessageTypesToLookup")
          .then(
            (response) => {
              resolve(response);
            },
    ...
    markMessageAsRead: async ({ dispatch, commit }, message) => {
      await axios
        .post("https://localhost:44379/api/messages/markMessageAsRead", {
          Id: message.id,
        })
        .then(() => {
    ...
    deleteMessage: async ({ dispatch, commit }, message) => {
      await axios
        .post("https://localhost:44379/api/messages/deleteMessage", {
          Id: message.id,
        })
        .then(() => {
          commit("RESET_UNREAD_MESSAGES_COUNT");
    ...
    getUsersToLookup: () => {
      return new Promise((resolve, reject) => {
        axios.get("https://localhost:44379/api/messages/getUsersToLookup").then(
          (response) => {
            resolve(response);
          },
          (error) => {
